...
const handleOrderNow = () => {
  ...
  const maxIndex = Math.max(0, filteredProducts.length - cardsToShow);
        ...
        <div className="max-w-7xl mx-auto">
          {filteredProducts.length > 0 ? (
            <div className="relative">
              ...
              <div className="overflow-hidden rounded-2xl">
                ...
                >
                  {filteredProducts.map((product, index) => (
                    ...
                    >
                      <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 ring-2 ring-[#C6B05D] ring-opacity-50 transform hover:scale-105">
                        <div className="relative">
                          <img
                            src={product.image}
                            alt={product.name}
                        ...
                        <div className="p-5 sm:p-6">
                          ...
                          <div className="flex flex-wrap justify-between items-center">
                            ...
                            <button onClick={handleOrderNow} className="mt-2 sm:mt-0 bg-[#C6B05D] text-white px-5 py-2 rounded-full hover:bg-[#a08f4d] transition-colors duration-300 font-medium">
                              Order Now
                            </button>
                          </div>
                        </div>
              ...
              {/* Navigation Buttons */}
              {filteredProducts.length > cardsToShow && (
                <>
                  <button
                    onClick={prevSlide}
                    className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 bg-white shadow-lg rounded-full p-3 hover:bg-[#F6E9BB] transition-all duration-300 z-10 border border-[#C6B05D]/20"
                  >
                    <ChevronLeft className="w-6 h-6 text-[#C6B05D]" />
                  </button>
                  <button
                    onClick={nextSlide}
                    className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 bg-white shadow-lg rounded-full p-3 hover:bg-[#F6E9BB] transition-all duration-300 z-10 border border-[#C6B05D]/20"
                  >
                    <ChevronRight className="w-6 h-6 text-[#C6B05D]" />
                  </button>
                </>
              )}
                  ...
                  {Array.from({ length: maxIndex + 1 }, (_, i) => (
                    <button
                      ...
                      className={`w-3 h-3 rounded-full transition-all duration-300 ${
                        i === currentIndex
                          ? 'bg-[#C6B05D] scale-125'
                          : 'bg-[#C6B05D]/30 hover:bg-[#C6B05D]/50'
                      }`}
                    />
                  ))}
                </div>
              ...
              <div className="text-center mt-6">
                <p className="text-[#C6B05D]/70 text-sm">
                  Showing {Math.min(currentIndex + cardsToShow, filteredProducts.length)} of {filteredProducts.length} items
                </p>
              </div>
            </div>
          ) : (
            <div className="text-center py-16">
              <p className="text-[#C6B05D]/70 text-lg">No items found for the selected filter.</p>
            </div>
          )}
        </div>
      </section>
